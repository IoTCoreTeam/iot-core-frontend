...
type UseControlDataTabsOptions = {
...
export function useControlDataTabs(options: UseControlDataTabsOptions = {}) {
  ...
  async function loadRegisteredControlUrls() {
    ...
    return withLoading(async () => {
      try {
        ...
        registeredControlUrlRows.value = rows
          .map(mapRegisteredControlUrlRow)
          .sort(sortByLatestThenId);
      } catch (error: any) {
  ...
  function mapCommandSetupRow(row: any): DeviceRow {
    ...
    const controlUrlId = row?.control_url_id
    ...
    const deletedAt = row?.deleted_at ?? controlUrl?.deleted_at ?? null;
    ...
    return withLoading(async () => {
      try {
        const listEndpoint = options.includeSoftDeleted
          ? `${base}/control-json-commands?include=all`
        ...
        const visibleRows = options.includeSoftDeleted
          ? rows
          : rows.filter(
              (row: any) =>
                row?.deleted_at == null &&
                row?.control_url?.deleted_at == null &&
                row?.control_url?.node_deleted_at == null &&
                row?.control_url?.gateway_deleted_at == null,
            );
        commandSetupRows.value = visibleRows
          .map(mapCommandSetupRow)
          .sort(sortByLatestThenId);
      } catch (error: any) {
